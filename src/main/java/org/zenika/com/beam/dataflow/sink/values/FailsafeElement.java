package org.zenika.com.beam.dataflow.sink.values;

import java.util.Objects;

import org.apache.avro.reflect.Nullable;
import org.apache.beam.sdk.coders.DefaultCoder;
import org.zenika.com.beam.dataflow.sink.coders.FailsafeElementCoder;

import com.google.common.base.MoreObjects;

/**
 * The {@link FailsafeElement} class holds the current value and original value
 * of a record within a pipeline. This class allows pipelines to not lose
 * valuable information about an incoming record throughout the processing of
 * that record. The use of this class allows for more robust dead-letter
 * strategies as the original record information is not lost throughout the
 * pipeline and can be output to a dead-letter in the event of a failure during
 * one of the pipelines transforms.
 */
...
public class FailsafeElement<OriginalT, CurrentT> {
	...
	public static <OriginalT, CurrentT> FailsafeElement<OriginalT, CurrentT> of(
		...
		return new FailsafeElement<>(other.originalPayload, other.payload).setErrorMessage(other.getErrorMessage())
	...
	public String toString() {
		return MoreObjects.toStringHelper(this).add("originalPayload", originalPayload).add("payload", payload)
				.add("errorMessage", errorMessage).add("stacktrace", stacktrace).toString();
	}
}
